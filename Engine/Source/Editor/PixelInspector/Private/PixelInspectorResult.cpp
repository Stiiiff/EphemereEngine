...
namespace PixelInspector
{
	...
	void PixelInspectorResult::DecodeBufferA(TArray<FColor> &BufferAValue)
	{
		if (BufferAValue.Num() > 0)
		{
			...
			Roughness = (float)(BufferAValue[0].A) / 255.0f;
		}
	}
	...
	void PixelInspectorResult::DecodeBufferA(TArray<FFloat16Color> &BufferAValue)
	{
		if (BufferAValue.Num() > 0)
		{
			...
			Roughness = BufferAValue[0].A.GetFloat();
		}
	}
	...
	EMaterialShadingModel PixelInspectorResult::DecodeShadingModel(float InPackedChannel)
	{
		...
		case PIXEL_INSPECTOR_SHADINGMODELID_LIT:
			return EMaterialShadingModel::MSM_Lit;
		case PIXEL_INSPECTOR_SHADINGMODELID_SKIN:
			return EMaterialShadingModel::MSM_Skin;
			case PIXEL_INSPECTOR_SHADINGMODELID_METAL:
			return EMaterialShadingModel::MSM_Metal;
		case PIXEL_INSPECTOR_SHADINGMODELID_FOLIAGE:
			return EMaterialShadingModel::MSM_Foliage;
	...
	void PixelInspectorResult::DecodeCustomData(FVector4 InCustomData)
	{
		...
		case EMaterialShadingModel::MSM_Lit:
		case EMaterialShadingModel::MSM_Metal:
		{
		...
		break;
		case EMaterialShadingModel::MSM_Clothing:
		{
