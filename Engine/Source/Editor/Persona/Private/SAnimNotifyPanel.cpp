...
public:
		...
		)
	{
		...
		Operation->Construct();
		Operation->CursorDecoratorWindow->GetNativeWindow()->SetDPIScaleFactor(1.0f);
...
FReply SAnimNotifyPanel::OnNotifyNodeDragStarted(TArray<TSharedPtr<SAnimNotifyNode>> NotifyNodes, TSharedRef<SWidget> Decorator, const FVector2D& ScreenCursorPos, const FVector2D& ScreenNodePosition, const bool bDragOnMarker)
{
	...
	float DPIScale = FPlatformApplicationMisc::GetDPIScaleFactorAtPoint(ScreenCursorPos.X, ScreenCursorPos.Y);
	FBox2D OverlayBounds(Nodes[0]->GetScreenPosition(), Nodes[0]->GetScreenPosition() + FVector2D(Nodes[0]->GetDurationSize() * DPIScale, 0.0f));
	for(int32 Idx = 1 ; Idx < Nodes.Num() ; ++Idx)
	{
		TSharedPtr<SAnimNotifyNode> Node = Nodes[Idx];
		FVector2D NodePosition = Node->GetScreenPosition();
		float NodeDuration = Node->GetDurationSize() * DPIScale;

		OverlayBounds += FBox2D(NodePosition, NodePosition + FVector2D(NodeDuration, 0.0f));
	}

	FVector2D OverlayOrigin = OverlayBounds.Min;
	const FVector2D OverlayExtents = OverlayBounds.GetSize() / DPIScale;

	FSlateRenderTransform InvTrackTransform = GetCachedGeometry().GetAccumulatedRenderTransform().Inverse();

	for(TSharedPtr<SAnimNotifyNode> Node : Nodes)
	{
		FVector2D OffsetFromFirst(Node->GetScreenPosition() - OverlayOrigin);

		Node->SetRenderTransform(FSlateRenderTransform(DPIScale, FVector2D(0.0, 0.0)));
		const float Width = Node->GetPaintSpaceGeometry().Size.GetMax();

		NodeDragDecoratorOverlay->AddSlot()
			.Padding(FMargin(static_cast<float>(OffsetFromFirst.X * DPIScale), static_cast<float>(OffsetFromFirst.Y * DPIScale), Width * 0.5, (DPIScale)*NotifyHeight))
			[
				Node->AsShared()
			];
	}
