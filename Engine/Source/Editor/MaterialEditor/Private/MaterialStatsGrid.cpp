...
/*==============================================================================================================*/
/* FStatsGridRow_NumVirtualTextureStacks functions*/

void FStatsGridRow_NumVirtualTextureStacks::CreateRow(TSharedPtr<FMaterialStats> StatsManager)
{
	// static string in the descriptor column
	TSharedPtr<FGridCell> HeaderCell = MakeShareable(new FGridCell_StaticString(TEXT("VT Stacks (Est.)"), TEXT("Virtual Texture Stacks (Est.)")));
	HeaderCell->SetColor(FLinearColor::Gray);
	HeaderCell->SetContentBold(true);
...
void FStatsGridRow_NumVirtualTextureStacks::AddPlatform(TSharedPtr<FMaterialStats> StatsManager, const TSharedPtr<FShaderPlatformSettings> Platform, const EMaterialQualityLevel::Type QualityLevel)
{
	// cell that will enumerate the total number of texture samples in this material
	const FString CellContent = FMaterialStatsUtils::MaterialQualityToShortString(QualityLevel);
	TSharedPtr<FGridCell_ShaderValue> Cell = MakeShareable(new FGridCell_ShaderValue(StatsManager, EShaderInfoType::VirtualTextureStackCount, ERepresentativeShader::Num, QualityLevel, Platform->GetPlatformShaderType()));

	const FName ColumnName = FMaterialStatsGrid::MakePlatformColumnName(Platform, QualityLevel);
	AddCell(ColumnName, Cell);
}

/*end FStatsGridRow_NumVirtualTextureStacks functions*/
/*==============================================================================================================*/
...
void FMaterialStatsGrid::BuildRowIds()
{
	...
	if (PlatformErrorsType != EGlobalErrorsType::GlobalPlatformErrors)
	{
		...
		BuildKeyAndInsert(ERowType::VirtualTextureStacks);
...
void FMaterialStatsGrid::BuildStaticRows()
{
	...
	{
		TSharedPtr<FStatsGridRow> Row = MakeShareable(new FStatsGridRow_NumVirtualTextureStacks());
		Row->CreateRow(StatsManager);
		StaticRows.Add(ERowType::VirtualTextureStacks, Row);
	}
	{
...
FString FGridCell_ShaderValue::InternalGetContent(bool bLongContent)
{
	...
	case EShaderInfoType::VirtualTextureStackCount:
		return bLongContent ? PlatformData.ShaderStatsInfo.VirtualTextureStackCount.StrDescriptionLong : PlatformData.ShaderStatsInfo.VirtualTextureStackCount.StrDescription;
	}
