...
FText FMetaSettingGatherer::ValueToString(EAntiAliasingMethod Value)
{
	...
	case AAM_SMAA:
		return LOCTEXT("AA_SMAA", "Subpixel Morphological Anti-Aliasing (SMAA)");
	default:
...
void FHardwareTargetingModule::GatherSettings(FMetaSettingGatherer& Builder)
{
...

	{
		...
		UE_META_SETTING_ENTRY(Builder, URendererSettings, bDefaultFeatureBloom, true);

		// Separate translucency
		UE_META_SETTING_ENTRY(Builder, URendererSettings, bSeparateTranslucency, true);
...

	{
		...
		UE_META_SETTING_ENTRY(Builder, USlateSettings, bExplicitCanvasChildZOrder, false);
	}
}
