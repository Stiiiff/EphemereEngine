...
TMap<FName, TArray<TSharedPtr<FTemplateItem>> >& SNewProjectWizard::FindTemplateProjects()
{
	...
	// Add blank template to empty categories
	{
		...
		BlankTemplate->Description = LOCTEXT("BlankProjectDescription", "A clean empty project with no code and default settings. You can use Engine/Templates folder for the template map and characters.");
		...
		Templates.Add(FName("Games")).Add(BlankTemplate);
	}
