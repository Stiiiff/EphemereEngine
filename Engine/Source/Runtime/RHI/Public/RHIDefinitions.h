...
#define DDPI_NUM_STATIC_SHADER_PLATFORMS 16
	...
	//---------------------------------------------------------------------------------
	//SP_VULKAN_SM6 = SP_StaticPlatform_Last + 1
...
{
	return IsVulkanPlatform(Platform) || FDataDrivenShaderPlatformInfo::GetIsHlslcc(Platform);
}
...
inline bool RHISupportsDualSourceBlending(const FStaticShaderPlatform Platform)
{
	...
	return IsFeatureLevelSupported(Platform, ERHIFeatureLevel::SM5) && (FDataDrivenShaderPlatformInfo::GetSupportsDualSourceBlending(Platform) || IsVulkanPlatform(Platform));
}
