		...
		ENQUEUE_RENDER_COMMAND(UpdateLightColorAndBrightness)(
			[LightSceneInfo, Scene, NewParameters](FRHICommandListImmediate& RHICmdList)
			{
				if( LightSceneInfo && LightSceneInfo->bVisible )
				{
					...
					Scene->bScenesPrimitivesNeedStaticMeshElementUpdate = Scene->bScenesPrimitivesNeedStaticMeshElementUpdate;
...
{
	if (WindSources.Num() == 0)
	{
		//don't update the wind if there are no wind sources
		//(except if there wind sources last frame - then we want to update the wind one last time)
		if (!bHadWindSourcesLastTick)
		{
			return;
		}
	}

	bHadWindSourcesLastTick = WindSources.Num() > 0;
