...
public:
	static bool ShouldCompilePermutation(const FMeshMaterialShaderPermutationParameters& Parameters)
	{
		...
		bShouldCache &= (false && false && IsTranslucentBlendMode(Parameters.MaterialParameters.BlendMode)) || !false;
...
protected:
	...
	static bool ShouldCompilePermutation(const FMeshMaterialShaderPermutationParameters& Parameters)
	{
		...
		return (false == false)
			...
			&& TBasePassVS<LightMapPolicyType>::ShouldCompilePermutation(Parameters);
	}

	static void ModifyCompilationEnvironment(const FMaterialShaderPermutationParameters& Parameters, FShaderCompilerEnvironment& OutEnvironment)
	{
		// Re-use vertex shader compilation environment
		TBasePassVS<LightMapPolicyType>::ModifyCompilationEnvironment(Parameters, OutEnvironment);
	}
};
...
public:
	...
	static bool ShouldCompilePermutation(const FMeshMaterialShaderPermutationParameters& Parameters)
	{
		...
		const bool IsSingleLayerWater = false;
	...
	)
{
	...
	if (bNeedsHSDS)
	{
		...
		ShaderTypes.AddShaderType<TBasePassHS<LightMapPolicyType>>();
