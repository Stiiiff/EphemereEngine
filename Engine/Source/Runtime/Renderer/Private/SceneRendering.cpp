...
void FViewInfo::Init()
{
	...
	VolumetricFogAlbedo = FVector::ZeroVector;
	...
	const int32 MaxShadowCascadeCountUpperBound = 10;
	...
	FViewUniformShaderParameters& ViewUniformShaderParameters) const
{
	...
	const FIntRect EffectiveViewRect = ViewRect;
	...
	ViewUniformShaderParameters.StereoPassIndex = 0;
...
{
	checkf(FeatureLevel >= ERHIFeatureLevel::SM5, TEXT("SM5 and above use RenderTarget for read back"));
...
{
	return InFeatureLevel >= ERHIFeatureLevel::SM5 && CVarAllowOcclusionQueries.GetValueOnRenderThread() != 0;
}
...
void FSceneRenderer::RenderFinish(FRDGBuilder& GraphBuilder, FRDGTextureRef ViewFamilyTexture)
{
...
#if !(UE_BUILD_SHIPPING || UE_BUILD_TEST)
	{
		...
		if (Scene->PlanarReflections.Num() > 0)
		{
			...
			const bool bShouldUseClipPlaneForPlanarReflection = (FeatureLevel >= ERHIFeatureLevel::SM5);
		...
		const bool bAnyWarning = bShowPrecomputedVisibilityWarning || bShowGlobalClipPlaneWarning || bShowSkylightWarning || bShowPointLightWarning 
			|| bShowDFAODisabledWarning || bShowShadowedLightOverflowWarning
			|| bShowSkinCacheOOM || bSingleLayerWaterWarning || bShowDFDisabledWarning || bShowNoSkyAtmosphereComponentWarning || bFxDebugDraw 
			|| bRealTimeSkyCaptureButNothingToCapture;
			...
			if (!View.bIsReflectionCapture && !View.bIsSceneCapture )
			{
				...
				if ((GAreScreenMessagesEnabled && !GEngine->bSuppressMapWarnings) && (bViewParentOrFrozen || bLocked || bAnyWarning))
				{
					...
					AddDrawCanvasPass(GraphBuilder, {}, View, Output,
						...
						bShowSkinCacheOOM, bSingleLayerWaterWarning, bShowNoSkyAtmosphereComponentWarning, bFxDebugDraw,
...
void AddResolveSceneColorPass(FRDGBuilder& GraphBuilder, const FViewInfo& View, FRDGTextureMSAA SceneColor)
{
	...
	else
	{
			...
			[&View, SceneColorTargetable, SceneColorFMask, NumSamples](FRHICommandList& RHICmdList)
		{
			...
			RHICmdList.SetScissorRect(true, View.ViewRect.Min.X, View.ViewRect.Min.Y,
