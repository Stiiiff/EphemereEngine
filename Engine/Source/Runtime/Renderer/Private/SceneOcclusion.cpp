...
int32 FOcclusionQueryHelpers::GetNumBufferedFrames(ERHIFeatureLevel::Type FeatureLevel)
{
	...
	return FMath::Clamp<int32>(NumBufferedQueriesVar->GetValueOnAnyThread() * NumGPUS, 1, (int32)FOcclusionQueryHelpers::MaxBufferedOcclusionFrames);
}
...
void FFrameBasedOcclusionQueryPool::AdvanceFrame(uint32 InOcclusionFrameCounter, uint32 InNumBufferedFrames)
{
	...
	FRHIRenderQuery*& ShadowOcclusionQuery)
{
	...
	else if (IntersectionMode == SOQ_NearPlaneVsShadowFrustum)
	{
		...
		const bool bIntersectsNearClippingPlane = ProjectedShadowInfo.ReceiverFrustum.IntersectSphere(
			...
			View.NearClippingDistance * UE_SQRT_3
			);
