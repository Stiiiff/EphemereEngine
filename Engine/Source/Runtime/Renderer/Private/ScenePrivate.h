...
public:
	 ...
	 *  waits on the potential jobs dispatched in Extension->PrepareView().
	 */
	...
	{
		return static_cast<const FViewInfo&>(View.Family->GetStereoEyeView());
	}
...
public:
	...
	/** Indicates if at least one wind source existed last tick (and the wind still needs to be updated because of that. */
	bool bHadWindSourcesLastTick = false;
			...
			bRenderSkyLight = bRenderSkyLight
				&& ((ReadOnlyCVARCache.bEnableStationarySkylight && SkyLight->bWantsStaticShadowing)
					|| (!SkyLight->bWantsStaticShadowing
						&& (IsAnyForwardShadingEnabled(GetShaderPlatform()))));
		}
