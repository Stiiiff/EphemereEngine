...
public:
	...
	virtual void DrawMesh(const FMeshBatch& Mesh, float ScreenSize) final override
	{
		if (Mesh.HasAnyDrawCalls())
		{
			...
			bool bUseSingleLayerWaterMaterial = false;
			bool bUseAnisotropy = Material.GetShadingModels().HasAnyShadingModel({MSM_Metal}) && Material.MaterialUsesAnisotropy_RenderThread();
...
void FPrimitiveSceneInfo::UpdateIndirectLightingCacheBuffer()
{
	if (bIndirectLightingCacheBufferDirty)
	{
		...
		if (IndirectLightingCacheAllocation && (Scene->IndirectLightingCache.IsInitialized() && IndirectLightingCacheAllocation->bHasEverUpdatedSingleSample))
		{
