	...
	for(int32 ViewIndex = 0;ViewIndex < Views.Num();ViewIndex++)
	{
		...
		if(ShouldRenderFog(*View.Family))
		{
			if (Scene->ExponentialFogs.Num() > 0)
			{
				...
				View.VolumetricFogAlbedo = FVector::ZeroVector; // unused by default
				if (bExpFogMatchesVolumetricFog)
				{
					...
					View.ExponentialFogColor = FVector(FogInfo.VolumetricFogEmissive);		// Emisive from ExpFog matches the VFog ExponentialFogColorParameter
					View.VolumetricFogAlbedo = FVector(FogInfo.VolumetricFogAlbedo);		// Albedo is now supported by ExpFog and matches the VFog
