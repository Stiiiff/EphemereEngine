...
public:
	...
	static void ModifyCompilationEnvironment(const FMaterialShaderPermutationParameters& Parameters, FShaderCompilerEnvironment& OutEnvironment)
	{
		...
		uint32 StencilEnablement = Parameters.MaterialParameters.bIsStencilTestEnabled ? 1 : 0;
		OutEnvironment.SetDefine(TEXT("MANUAL_STENCIL_TEST"), StencilEnablement);
	}
};
	...
	const UMaterialInterface* MaterialInterface)
{
	...
	const bool bCompositeWithInputAndDecode = false && bCompositeWithInput;
	...
	else if (!DepthStencilTexture && false && Material->IsStencilTestEnabled()) // we have to set a default texture for MobileStencilTexture and override the MobileStencilValueRef to make all function to pass the stencil test
	{
