...
{
	return EV100ToLuminance(LuminanceMaxFromLensAttenuation(), View.Family->ExposureSettings.FixedEV100);
}
...
float CalculateManualAutoExposure(const FViewInfo& View, bool bForceDisablePhysicalCamera)
{
	...
	float FoundLuminance = EV100ToLuminance(LuminanceMaxFromLensAttenuation(), PhysicalCameraEV100);
...
FEyeAdaptationParameters GetEyeAdaptationParameters(const FViewInfo& View, ERHIFeatureLevel::Type MinFeatureLevel)
{
	...
	const float HistogramLogMax = EV100ToLog2(LuminanceMaxFromLensAttenuation(), Settings.HistogramLogMax);
	const float HistogramLogMin = FMath::Min(EV100ToLog2(LuminanceMaxFromLensAttenuation(), Settings.HistogramLogMin), HistogramLogMax - 1);
	...
	else if (EngineShowFlags.EyeAdaptation && View.GetFeatureLevel() >= MinFeatureLevel)
	{
		...
		{
			MinWhitePointLuminance = EV100ToLuminance(LuminanceMaxFromLensAttenuation(), Settings.AutoExposureMinBrightness);
			MaxWhitePointLuminance = EV100ToLuminance(LuminanceMaxFromLensAttenuation(), Settings.AutoExposureMaxBrightness);
		}
	}
	else
	{
	...
	Parameters.LuminanceMax = LuminanceMaxFromLensAttenuation();
