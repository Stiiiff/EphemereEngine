...
public:
	...
	bool SupportsShadowStencilCulling(FStaticShaderPlatform ShaderPlatform) const;
	bool UseShadowStencilCulling(const FViewInfo& View) const;

	/**
	 * Projects the shadow onto the scene for a particular view.
	 */
	void RenderProjection(FRHICommandListImmediate& RHICmdList, int32 ViewIndex, const class FViewInfo* View, const class FSceneRenderer* SceneRender, bool bProjectingForForwardShading, const struct FHairStrandsVisibilityData* HairVisibilityData, const struct FHairStrandsMacroGroupDatas* HairMacroGroupData) const;
	 ...
	 */
	void AddSubjectPrimitive(FPrimitiveSceneInfo* PrimitiveSceneInfo, TArray<FViewInfo>* ViewArray, ERHIFeatureLevel::Type FeatureLevel);
...
public:
	...
	void Set(FRHICommandList& RHICmdList, FShader* Shader, const FSceneView& View, const FProjectedShadowInfo* ShadowInfo, const FHairStrandsVisibilityData* HairVisibilityData, bool bUseFadePlane)
	{
 ...
 */
template<uint32 Quality, bool bUseFadePlane = false, bool bUseTransmission = false, bool SubPixelShadow = false>
...
public:
		...
		const FProjectedShadowInfo* ShadowInfo)
	{
		...
		ProjectionParameters.Set(RHICmdList, this, View, ShadowInfo, HairVisibilityData, bUseFadePlaneEnable);
