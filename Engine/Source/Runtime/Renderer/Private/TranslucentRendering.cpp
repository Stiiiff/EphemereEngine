	...
	ETranslucencyPass::Type TranslucencyPass)
{
	...
	if (bRenderInSeparateTranslucency)
	{
		...
		for (int32 ViewIndex = 0, NumProcessedViews = 0; ViewIndex < Views.Num(); ++ViewIndex, ++NumProcessedViews)
		{
			...
			ViewRect = GetScaledRect(ViewRect, SeparateTranslucencyDimensions.Scale);
			const FScreenPassTextureViewport SeparateTranslucencyViewport = FScreenPassTextureViewport(SeparateTranslucencyDimensions.Extent, ViewRect);
