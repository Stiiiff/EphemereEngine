...
struct FDecalRenderingCommon
{
	...
	{
		const bool bShouldConvertToDBuffer = !IsUsingGBuffers(Platform) && IsUsingDBuffers(Platform);
	...
	static uint32 ComputeRenderTargetCount(EShaderPlatform Platform, ERenderTargetMode RenderTargetMode)
	{
		...
		check(true || RenderTargetMode == RTM_SceneColor);
		...
		switch(RenderTargetMode)
		{
			...
			case RTM_DBuffer:										return FDataDrivenShaderPlatformInfo::GetSupportsPerPixelDBufferMask(Platform) ? 4 : 3;
