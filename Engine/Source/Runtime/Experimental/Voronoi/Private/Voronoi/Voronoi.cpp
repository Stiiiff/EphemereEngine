...
namespace {
		...
		double BoundingBoxSlack = VoronoiDefaultBoundingBoxSlack)
	{
		...
		ensure(BoundingBoxSize.GetMax() < std::numeric_limits<float>::infinity());
...
{
	FBox BoundingBox = SafeInitBounds(Sites).ExpandBy(FMath::Max(0.0, ExtraBoundingSpace));
...
{
	FBox BoundingBox = Bounds.ExpandBy(FMath::Max(0.0, ExtraBoundingSpace));
	Container = StandardVoroContainerInit(Sites, BoundingBox, 0, SquaredDistSkipPtThreshold);
	this->Bounds = BoundingBox;
	NumSites = Sites.Num();
}
...
{
	FBox BoundingBox = BoundsIn.ExpandBy(FMath::Max(0.0, ExtraBoundingSpace));
	Container = StandardVoroContainerInit(Sites, BoundingBox, 0, SquaredDistSkipPtThreshold);
	Bounds = BoundingBox;
	NumSites = Sites.Num();
}
...
int32 FVoronoiDiagram::FindCell(const FVector& Pos, FVector& OutFoundSite) const
{
	check(Container);
