...
bool VORONOI_API VoronoiNeighbors(const TArrayView<const FVector> &Sites, TArray<TArray<int>> &Neighbors, bool bExcludeBounds = true, double SquaredDistSkipPtThreshold = KINDA_SMALL_NUMBER);
bool VORONOI_API GetVoronoiEdges(const TArrayView<const FVector> &Sites, const FBox& Bounds, TArray<TTuple<FVector, FVector>> &Edges, TArray<int32> &CellMember, double SquaredDistSkipPtThreshold = KINDA_SMALL_NUMBER);
...
public:
	 ...
	 */
	VORONOI_API FVoronoiDiagram(const TArrayView<const FVector>& Sites, double ExtraBoundingSpace, double SquaredDistSkipPtThreshold = KINDA_SMALL_NUMBER);
	/**
	 ...
	 *										(If you know there will be no duplicate sites, can set to zero for faster perf.)
	 */
	VORONOI_API FVoronoiDiagram(const TArrayView<const FVector>& Sites, const FBox &Bounds, double ExtraBoundingSpace, double SquaredDistSkipPtThreshold = KINDA_SMALL_NUMBER);
	 ...
	 *										(If you know there will be no duplicate sites, can set to zero for faster perf.)
	 */
	VORONOI_API void Initialize(const TArrayView<const FVector>& Sites, const FBox& Bounds, double ExtraBoundingSpace, double SquaredDistSkipPtThreshold = KINDA_SMALL_NUMBER);

	VORONOI_API ~FVoronoiDiagram();

	static VORONOI_API FBox GetBounds(const TArrayView<const FVector>& Sites, double ExtraBoundingSpace = KINDA_SMALL_NUMBER);
	...
	VORONOI_API void ComputeAllCells(TArray<FVoronoiCellInfo> &AllCells);

	VORONOI_API void ComputeAllNeighbors(TArray<TArray<int32>>& AllNeighbors, bool bExcludeBounds = true);

	VORONOI_API void ComputeCellEdges(TArray<TTuple<FVector, FVector>>& Edges, TArray<int32>& CellMember);
	 ...
	 */
	VORONOI_API int32 FindCell(const FVector& Pos, FVector& OutFoundSite) const;

	int32 FindCell(const FVector& Pos) const
	{
		FVector OutFoundSite;
		return FindCell(Pos, OutFoundSite);
	}
...
public:
	...
	TPair<int32, int32> ClosestTwoIDs(const FVector& Sample, FVoronoiComputeHelper& ComputeHelper)
	{
		...
		FVector CloseSite;
		ToRet.Key = Diagram.FindCell(Sample, CloseSite);
		if (ToRet.Key < 0)
		{
			return ToRet;
		}
	...
	{
		FVector CloseSite;
		int32 Cell = Diagram.FindCell(Sample, CloseSite);
		if (Cell < 0)
		{
			return InvalidValue;
		}
	...
	{
		return Diagram.FindCell(Sample);
	}
};
