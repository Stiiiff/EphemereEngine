...
public:
	void Serialize(FChaosArchive& Ar)
	{
		...
		if (Ar.CustomVer(FPhysicsObjectVersion::GUID) >= FPhysicsObjectVersion::AddCollisionConstraintFlag)
		{
			Ar << MCollisionConstraintFlag;
		}

		if (Ar.CustomVer(FPhysicsObjectVersion::GUID) >= FPhysicsObjectVersion::AddDisabledFlag)
		{
			Ar << bDisabled;
		}
		
		if(Ar.CustomVer(FPhysicsObjectVersion::GUID) >= FPhysicsObjectVersion::AddChaosMaxLinearAngularSpeed)
		{
