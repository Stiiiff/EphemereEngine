...
public:
	...
	CHAOS_API virtual void Serialize(FChaosArchive& Ar) override
	{
		...
		if (Ar.IsSaving())
		{
			IArray.Reserve(MI.Num());
			InvIArray.Reserve(MInvI.Num());
			for (int32 Idx = 0; Idx < IArray.Num(); ++Idx)
			{
				IArray.Add(PMatrix<T, d, d>(MI[Idx]));
				InvIArray.Add(PMatrix<T, d, d>(MInvI[Idx]));
			}
		}
		...
		if (Ar.IsLoading())
		{
			MI.Reserve(IArray.Num());
			MInvI.Reserve(InvIArray.Num());
			for (int32 Idx = 0; Idx < IArray.Num(); ++Idx)
			{
