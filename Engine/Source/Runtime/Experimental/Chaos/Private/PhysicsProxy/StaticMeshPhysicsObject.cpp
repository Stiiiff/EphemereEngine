...
void FStaticMeshPhysicsProxy::CreateRigidBodyCallback(FParticlesType& Particles)
{
	if(!bInitializedState && Parameters.bSimulating)
	{
		...
		const FVector Inertia((Chaos::FRealSingle)(Parameters.Mass * (SideSquared.Y + SideSquared.Z) / 12.f), (Chaos::FRealSingle)(Parameters.Mass * (SideSquared.X + SideSquared.Z) / 12.f), (Chaos::FRealSingle)(Parameters.Mass * (SideSquared.X + SideSquared.Y) / 12.f));
