	...
	struct FDefaultCollectionFactory : public ISpatialAccelerationCollectionFactory
	{
		...
		virtual TUniquePtr<ISpatialAcceleration<FAccelerationStructureHandle, FReal, 3>> CreateAccelerationPerBucket_Threaded(const TConstParticleView<FSpatialAccelerationCache>& Particles, uint16 BucketIdx, bool ForceFullBuild, bool bDynamicTree) override
		{
			...
			case 0:
			{
				...
				{
					const int MaxNumToProcess = ForceFullBuild ? 0 : ConfigSettings.IterationsPerTimeSlice;
					
					if (bDynamicTree)
					{
						return MakeUnique<AABBDynamicTreeType>(Particles, ConfigSettings.MaxChildrenInLeaf, ConfigSettings.MaxTreeDepth, ConfigSettings.MaxPayloadSize, MaxNumToProcess, bDynamicTree);
					}

					return MakeUnique<AABBTreeType>(Particles, ConfigSettings.MaxChildrenInLeaf, ConfigSettings.MaxTreeDepth, ConfigSettings.MaxPayloadSize, MaxNumToProcess, bDynamicTree);
				}
				else if (ConfigSettings.BroadphaseType == 4 || ConfigSettings.BroadphaseType == 2)
				{
