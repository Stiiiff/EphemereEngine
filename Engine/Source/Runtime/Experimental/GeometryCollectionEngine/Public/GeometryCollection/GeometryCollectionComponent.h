...
public:
	...
	COPY_ON_WRITE_ATTRIBUTE(TArray<FVector2D>, UVs, FGeometryCollection::VerticesGroup)		//GetUVsArray
	...
	UPhysicalMaterial * PhysicalMaterialOverride_DEPRECATED;
...
protected:
	...
	/** Populate the dynamic data with required GC attributes, return true if attributes were updated */
	bool SetDynamicDataAttributes(FGeometryCollectionDynamicData* DynamicData) const;
	...
	void CreateRemovalAttributesIfMissing(bool bAllActiveByDefault);
...
private:
	...
	/** True if removal update code needs to be executed. */
	bool bNeedRemovalUpdate;
