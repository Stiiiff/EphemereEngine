...
{
	uint32 CpuID = FCPUIDQueriedData::GetCPUInfo();
	uint32 SteppingID = CpuID & 0x0F;
	uint32 ModelID = (CpuID >> 4) & 0x0F;
	uint32 FamilyID = (CpuID >> 8) & 0x0F;

	if (FamilyID == 0x06 || FamilyID == 0x0F)
	{
		uint32 ExtendedModelID = (CpuID >> 16) & 0x0F;
		ModelID = (ExtendedModelID << 4) + ModelID;
	}

	if (FamilyID == 0x0F)
	{
		uint32 ExtendedFamilyID = (CpuID >> 20) & 0xFF;
		FamilyID = FamilyID + ExtendedFamilyID;
	}

	// Repack Family (12 bits), Model (8 bits), Stepping (4 bits) into a single 32-bit integer.
	return (FamilyID << 12) | (ModelID << 4) | SteppingID;
}

uint32 FWindowsPlatformMisc::GetCPUMicrocodeRevision()
{
	// The microcode revision number is only available in a model-specific register, so we can't read it from userland. However, Windows
	// seems to store the value in the registry at boot time, so we can try to get it from there.
	...
	uint32 MicrocodeVersion = 0;

	uint32 MicrocodeVersionBuf[2];
	::DWORD Size = 0, ValueType = 0;
	if (RegQueryValueEx(Key, TEXT("Update Revision"), NULL, &ValueType, NULL, &Size) == ERROR_SUCCESS && ValueType == REG_BINARY && Size <= sizeof(MicrocodeVersionBuf))
	{
		if (RegQueryValueEx(Key, TEXT("Update Revision"), NULL, NULL, (LPBYTE)MicrocodeVersionBuf, &Size) == ERROR_SUCCESS)
		{
			if (Size == 8)
			{
				// On older Windows versions the registry key contains the full 64-bit value of the IA32_BIOS_SIGN_ID MSR. In that case,
				// the microcode revision number is in the high 32 bits.
				MicrocodeVersion = MicrocodeVersionBuf[1];
			}
			else if (Size == 4)
			{
				// In most cases, Windows already shifts the value and stores only the 32-bit microcode revision number.
				MicrocodeVersion = MicrocodeVersionBuf[0];
			}
		}
	}
