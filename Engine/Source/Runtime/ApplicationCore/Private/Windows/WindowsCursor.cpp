...
FWindowsCursor::FWindowsCursor()
{
	...
	for( int32 CurCursorIndex = 0; CurCursorIndex < EMouseCursor::TotalCursorCount; ++CurCursorIndex )
	{
		...
		case EMouseCursor::None:
			if (bUseInvisibleCursorForNoneCursorType)
			{
	...
	SetTypeInternal( EMouseCursor::Default );
}
...
void FWindowsCursor::SetType( const EMouseCursor::Type InNewCursor )
{
	...
	if (CurrentType == InNewCursor)
	{
		return;
	}

	SetTypeInternal(InNewCursor);
}
...
void FWindowsCursor::SetTypeShape(EMouseCursor::Type InCursorType, void* InCursorHandle)
{
	...
	{
		SetTypeInternal(CurrentType);
	}
}

void FWindowsCursor::SetTypeInternal(const EMouseCursor::Type InNewCursor)
{
	CurrentType = InNewCursor;

	if (CursorOverrideHandles[CurrentType])
	{
		::SetCursor(CursorOverrideHandles[CurrentType]);
	}
	else
	{
		::SetCursor(CursorHandles[CurrentType]);
	}
}