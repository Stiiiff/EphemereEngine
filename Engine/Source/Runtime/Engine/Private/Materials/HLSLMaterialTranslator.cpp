...
bool FHLSLMaterialTranslator::Translate()
{
	STAT(double HLSLTranslateTime = 0);
	{
		...
		const bool bDBufferAllowed = bDBufferSupported;
...
void FHLSLMaterialTranslator::GetMaterialEnvironment(EShaderPlatform InPlatform, FShaderCompilerEnvironment& OutEnvironment)
{
	...
	{	
		...
		{
			OutEnvironment.SetDefine(TEXT("MATERIAL_SHADINGMODEL_LIT"), TEXT("1"));
			NumSetMaterials++;
		}
		if (ShadingModels.HasShadingModel(MSM_Skin))
		{
			OutEnvironment.SetDefine(TEXT("MATERIAL_SHADINGMODEL_SKIN"), TEXT("1"));
			NumSetMaterials++;
		}
		if (ShadingModels.HasShadingModel(MSM_Metal))
		{
			OutEnvironment.SetDefine(TEXT("MATERIAL_SHADINGMODEL_METAL"), TEXT("1"));
			NumSetMaterials++;
		}
		if (ShadingModels.HasShadingModel(MSM_Foliage))
		{
			OutEnvironment.SetDefine(TEXT("MATERIAL_SHADINGMODEL_FOLIAGE"), TEXT("1"));
			NumSetMaterials++;
		}
		...
		if (ShadingModels.HasShadingModel(MSM_Clothing))
		{
		...
		bool bSingleLayerWaterUsesSimpleShading = false;
		...
		if (NumSetMaterials == 0)
		{
			...
			UE_LOG(LogMaterial, Warning, TEXT("Unknown material shading model(s). Setting to MSM_Lit"));
			OutEnvironment.SetDefine(TEXT("MATERIAL_SHADINGMODEL_LIT"),TEXT("1"));
		}
	}
	else
	...
	)
{
	...
	switch( SamplerType )
	{
		...
		case SAMPLERTYPE_PackedNormal:
		case SAMPLERTYPE_VirtualPackedNormal:
			// Packed normal maps need to be unpacked in the pixel shader.
			SampleCode = FString::Printf(TEXT("UnpackCustomNormalMap(%s)"), *SampleCode);
			break;
...
{
	if (false && ShaderFrequency == SF_Vertex)
	{
