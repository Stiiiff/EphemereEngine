...
void UMaterialExpression::ConnectToPreviewMaterial(UMaterial* InMaterial, int32 OutputIndex)
{
	if (InMaterial && OutputIndex >= 0 && OutputIndex < Outputs.Num())
	{
		...
		{
			InMaterial->SetShadingModel(MSM_Lit);
...
EMaterialSamplerType UMaterialExpressionTextureBase::GetSamplerTypeForTexture(const UTexture* Texture, bool ForceNoVT)
{
	if (Texture)
	{
		...
		switch (Texture->CompressionSettings)
		{
			...
			case TC_BC7:
				return bVirtual ? SAMPLERTYPE_VirtualPackedNormal : SAMPLERTYPE_PackedNormal;
			case TC_Grayscale:
...
static void BuildPropertyToIOIndexMap()
{
	if (PropertyToIOIndexMap.Num() == 0)
	{
		...
		PropertyToIOIndexMap.Add(MP_Refraction,				16);
		PropertyToIOIndexMap.Add(MP_CustomizedUVs0,			17);
		PropertyToIOIndexMap.Add(MP_CustomizedUVs1,			18);
		PropertyToIOIndexMap.Add(MP_CustomizedUVs2,			19);
		PropertyToIOIndexMap.Add(MP_CustomizedUVs3,			20);
		PropertyToIOIndexMap.Add(MP_CustomizedUVs4,			21);
		PropertyToIOIndexMap.Add(MP_CustomizedUVs5,			22);
		PropertyToIOIndexMap.Add(MP_CustomizedUVs6,			23);
		PropertyToIOIndexMap.Add(MP_CustomizedUVs7,			24);
		PropertyToIOIndexMap.Add(MP_PixelDepthOffset,		25);
		...
		for (int32 ExpressionIndex = 0; ExpressionIndex < ExpressionCount; ++ExpressionIndex)
		{
			...
			if (TextureExpression && TextureExpression->Texture)
			{
				...
				case TC_Normalmap:
					...
					break;
				case TC_BC7:
					TextureExpression->SamplerType = SAMPLERTYPE_PackedNormal;
					break;
				case TC_Grayscale:
