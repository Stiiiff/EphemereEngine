...
bool FMaterial::NeedsGBuffer() const
{
	...
	if (FDataDrivenShaderPlatformInfo::GetOverrideFMaterial_NeedsGBufferEnabled(GMaxRHIShaderPlatform)) 
	{
		return true;
	}
...
{
	const bool bIsMaterialEnabled = false;
	...
	) const
{
	...
	OutEnvironment.SetDefine(TEXT("MATERIAL_USE_ALPHA_TO_COVERAGE"), IsUsingAlphaToCoverage() ? 1 : 0);
...
FString FMaterialResource::GetMaterialUsageDescription() const
{
	...
	BaseDescription += FString::Printf(
		...
		TEXT(", Usage={"),
		(int32)IsSpecialEngineMaterial(), (int32)IsTwoSided(), (int32)IsMasked(), (int32)IsDistorted(), (int32)WritesEveryPixel(), (int32)MaterialMayModifyMeshPosition()
		);
...
int32 FMaterialAttributeDefintion::CompileDefaultValue(FMaterialCompiler* Compiler)
{
	...
	if (Property == MP_SubsurfaceColor && Compiler->GetMaterialShadingModels().HasShadingModel(MSM_Foliage))
	{
...
void FMaterialAttributeDefinitionMap::InitializeAttributeMap()
{
	...
	Add(FGuid(0x57C3A161, 0x7F064296, 0xB00B24A5, 0xA496F34C), TEXT("ShadingShape"),	MP_ShadingShape,	MCT_Float,	FVector4(1, 0, 0, 0), SF_Vertex);
...
FText FMaterialAttributeDefinitionMap::GetAttributeOverrideForMaterial(const FGuid& AttributeID, UMaterial* Material)
{
	...
	case MP_ShadingShape:
		return LOCTEXT("ShadingShape", "ShadingShape");
