...
{
	return Parent ? Parent->GetShadingModels() : MSM_Lit;
}
			...
			if( bHasPropertyOverrides )
			{
				...
				if( Ar.UE4Ver() >= VER_UE4_MATERIAL_INSTANCE_BASE_PROPERTY_OVERRIDES_PHASE_2 )
				{
					...
					if( Ar.UE4Ver() < VER_UE4_STATIC_SHADOW_DEPTH_MAPS )
					{
						...
						{
							case MSM_Unlit:			BasePropertyOverrides.ShadingModel = MSM_Lit; break;
							case MSM_Lit:	BasePropertyOverrides.ShadingModel = MSM_Unlit; break;
						}
					}
				}
			}
