...
bool UGameViewportClient::TryToggleFullscreenOnInputKey(FKey Key, EInputEvent EventType)
{
	if ((Key == EKeys::Enter && EventType == EInputEvent::IE_Pressed && FSlateApplication::Get().GetModifierKeys().IsAltDown() && GetDefault<UInputSettings>()->bAltEnterTogglesFullscreen)
		|| (IsRunningGame() && Key == EKeys::F11 && EventType == EInputEvent::IE_Pressed && GetDefault<UInputSettings>()->bF11TogglesFullscreen && !(FSlateApplication::Get().GetModifierKeys().IsControlDown() || FSlateApplication::Get().GetModifierKeys().IsAltDown())))
	{
...
void UGameViewportClient::Draw(FViewport* InViewport, FCanvas* SceneCanvas)
{
	...
	for (FLocalPlayerIterator Iterator(GEngine, MyWorld); Iterator; ++Iterator)
	{
		...
		if (LocalPlayer)
		{
			...
			for (int32 i = 0; i < NumViews; ++i)
			{
				...
				FSceneView* View = LocalPlayer->CalcSceneView(&ViewFamily, ViewLocation, ViewRotation, InViewport, nullptr);
	...
	{
		if (bDisableWorldRendering || true) // TotalArea computation does not work correctly for stereoscopic views
		{
