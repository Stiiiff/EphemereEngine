			...
			if (ValidSpriteCount > 0)
			{
				...
				for (int32 SpriteIndex = 1; SpriteIndex < ValidSpriteCount + 1; SpriteIndex++)
				{
						...
						CurrentBlendMode != Sprites[SpriteIndex].BlendMode)
					{
						...
						PrepareShaders(RHICmdList, GraphicsPSOInit, StencilRef, FeatureLevel, CurrentBlendMode, Transform, bNeedToSwitchVerticalAxis, BatchedElementParameters, CurrentTexture, bHitTesting, Gamma, NULL, &View);
						RHICmdList.SetStreamSource(0, VertexBufferRHI, 0);
