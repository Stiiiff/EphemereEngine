...
FReply FSceneViewport::OnMouseButtonDown( const FGeometry& InGeometry, const FPointerEvent& InMouseEvent )
{
	...
	if (ViewportClient && GetSizeXY() != FIntPoint::ZeroValue)
	{
		...
		{
			if (!ViewportClient->InputKey(FInputKeyEventArgs(this, InMouseEvent.GetUserIndex(), InMouseEvent.GetEffectingButton(), IE_Pressed, 1.0f)))
			{
				CurrentReplyState = FReply::Unhandled();
			}
		}
...
FReply FSceneViewport::OnMouseButtonUp( const FGeometry& InGeometry, const FPointerEvent& InMouseEvent )
{
	...
	if( ViewportClient && GetSizeXY() != FIntPoint::ZeroValue )
	{
		if (!ViewportClient->InputKey(FInputKeyEventArgs(this, InMouseEvent.GetUserIndex(), InMouseEvent.GetEffectingButton(), IE_Released, 1.0f)))
		{
			CurrentReplyState = FReply::Unhandled(); 
		}
...
FReply FSceneViewport::OnMouseWheel( const FGeometry& InGeometry, const FPointerEvent& InMouseEvent )
{
	...
	if( ViewportClient  && GetSizeXY() != FIntPoint::ZeroValue  )
	{
		...
		ViewportClient->InputKey(FInputKeyEventArgs(this, InMouseEvent.GetUserIndex(), ViewportClientKey, IE_Pressed, 1.0f));
		ViewportClient->InputKey(FInputKeyEventArgs(this, InMouseEvent.GetUserIndex(), ViewportClientKey, IE_Released, 1.0f));
...
FReply FSceneViewport::OnMouseButtonDoubleClick( const FGeometry& InGeometry, const FPointerEvent& InMouseEvent )
{
	...
	if( ViewportClient && GetSizeXY() != FIntPoint::ZeroValue  )
	{
		...
		if (!ViewportClient->InputKey(FInputKeyEventArgs(this, InMouseEvent.GetUserIndex(), InMouseEvent.GetEffectingButton(), IE_DoubleClick, 1.0f)))
		{
			CurrentReplyState = FReply::Unhandled(); 
		}
	}
...
FReply FSceneViewport::OnKeyDown( const FGeometry& InGeometry, const FKeyEvent& InKeyEvent )
{
	...
	if (Key.IsValid())
	{
		...
		if (ViewportClient && GetSizeXY() != FIntPoint::ZeroValue)
		{
			...
			if (!ViewportClient->InputKey(FInputKeyEventArgs(this, InKeyEvent.GetUserIndex(), Key, InKeyEvent.IsRepeat() ? IE_Repeat : IE_Pressed, 1.0f)))
			{
				CurrentReplyState = FReply::Unhandled();
			}
		}
...
FReply FSceneViewport::OnKeyUp( const FGeometry& InGeometry, const FKeyEvent& InKeyEvent )
{
	...
	if (Key.IsValid())
	{
		...
		if (ViewportClient && GetSizeXY() != FIntPoint::ZeroValue)
		{
			...
			if (!ViewportClient->InputKey(FInputKeyEventArgs(this, InKeyEvent.GetUserIndex(), Key, IE_Released, 1.0f)))
			{
				CurrentReplyState = FReply::Unhandled();
			}
		}
...
void FSceneViewport::EnqueueBeginRenderFrame(const bool bShouldPresent)
{
	...
	// is enabled.
	if (!IsValidRef(ViewportRHI) && (!UseSeparateRenderTarget()))
	{
		// Get the viewport for this window from the renderer so we can render directly to the backbuffer
		FSlateRenderer* Renderer = FSlateApplication::Get().GetRenderer();
		if (ViewportWidget.IsValid())
...
void FSceneViewport::EndRenderFrame(FRHICommandListImmediate& RHICmdList, bool bPresent, bool bLockToVsync)
{
	...
	else
	{
		...
		bool bShouldUnsetTargets = !(false && !IsPCPlatform(GMaxRHIShaderPlatform));
