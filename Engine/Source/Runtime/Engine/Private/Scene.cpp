...
float CalculateEyeAdaptationExposureVersionUpdate(const FPostProcessSettings& Settings)
{
	...
	const float PrevLuminanceMax = 1.2f;
...
void FPostProcessSettings::PostSerialize(const FArchive& Ar)
{
	if (Ar.IsLoading())
	{
		...
		if (RenderingObjectVersion < FRenderingObjectVersion::AutoExposureChanges)
		{
			...
			if (bIsAnyNonDefault)
			{
				const float ExtraAutoExposureBias = CalculateEyeAdaptationExposureVersionUpdate(*this);
