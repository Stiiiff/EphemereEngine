...
ULocalPlayer::ULocalPlayer(FVTableHelper& Helper)
	: Super(Helper)
	...
	class FViewElementDrawer* ViewDrawer)
{
	...
	if (GetProjectionData(Viewport, /*inout*/ ViewInitOptions) == false)
	{
	...
	class FViewElementDrawer* ViewDrawer)
{
	...
	if (!CalcSceneViewInitOptions(ViewInitOptions, Viewport, ViewDrawer))
	{
		return nullptr;
	}
...

	{
		...
		if (PlayerController->PlayerCameraManager)
		{
			...
			checkSlow(CameraAnimPPBlendWeights && CameraAnimPPSettings);
...
bool ULocalPlayer::GetProjectionData(FViewport* Viewport, FSceneViewProjectionData& ProjectionData) const
{
	// If the actor
