		...
		if (DefaultArrayValueIndex < DefaultArrayValueHelper.Num())
		{
			...
			if (FStructProperty* InnerStructProperty = CastField<FStructProperty>(ArrayProperty->Inner))
			{
				...
				*CurrentArrayNodePtr = new FCustomPropertyListNode(ArrayProperty, ArrayValueIndex);
				...
				UScriptStruct::ICppStructOps* CppStructOps = nullptr;
				if (InnerStructProperty->Struct)
				{
					CppStructOps = InnerStructProperty->Struct->GetCppStructOps();
				}
				...
				if (!CppStructOps || !CppStructOps->HasIdentical() || !CppStructOps->Identical(ArrayPropertyValue, DefaultArrayPropertyValue, PortFlags, bIsIdentical))
				{
					// Recursively gather up all struct fields that differ and assign to the array item value node's sub property list.
					bIsIdentical = !BuildCustomPropertyListForPostConstruction((*CurrentArrayNodePtr)->SubPropertyList, InnerStructProperty->Struct, ArrayPropertyValue, DefaultArrayPropertyValue);
				}
