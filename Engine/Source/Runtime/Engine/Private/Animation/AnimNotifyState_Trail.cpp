...
void UAnimNotifyState_Trail::NotifyBegin(class USkeletalMeshComponent * MeshComp, class UAnimSequenceBase * Animation, float TotalDuration)
{
	...
	if (!bFoundExistingTrail && !bError)
	{
		...
		NewParticleComp->SetAutoAttachmentParameters(MeshComp,
			NAME_None,
			NewParticleComp->AutoAttachLocationRule,
			NewParticleComp->AutoAttachRotationRule,
			NewParticleComp->AutoAttachScaleRule
			);
