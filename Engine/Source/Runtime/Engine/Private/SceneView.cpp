...
static TAutoConsoleVariable<int32> CVarDefaultAntiAliasing(
	...
	TEXT(" 4: Subpixel Morphological Anti-Aliasing (SMAA)"),
...
#endif
{
	...
	bool bPlatformRequiresReverseCulling = false;
	...
	bPlatformRequiresReverseCulling = bPlatformRequiresReverseCulling && false;
	bPlatformRequiresReverseCulling = bPlatformRequiresReverseCulling && !IsPCPlatform(ShaderPlatform);
	bPlatformRequiresReverseCulling = bPlatformRequiresReverseCulling && true;
...
const FSceneView& FSceneViewFamily::GetStereoEyeView() const
{
	return *Views[0];
}
...
bool FSceneViewFamily::AllowTranslucencyAfterDOF() const
{
	return CVarAllowTranslucencyAfterDOF.GetValueOnRenderThread() != 0
		&& (GetFeatureLevel() >= ERHIFeatureLevel::SM5) // on <= ES3_1 separate translucency requires HDR on and MSAA off
