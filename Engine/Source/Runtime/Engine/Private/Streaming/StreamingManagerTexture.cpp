...
bool FRenderAssetStreamingManager::StreamOutRenderAssetData( int64 RequiredMemorySize )
{
	...
	for (int32 Idx = 0; Idx < StreamingRenderAssets.Num(); ++Idx)
	{
		...
		if (StreamingRenderAsset.IsMaxResolutionAffectedByGlobalBias() && (!StreamingRenderAsset.IsMesh()))
		{
...
bool FRenderAssetStreamingManager::IsParallelForAllowedDuringIncrementalUpdate(int32 NumElementsToProcess, int32& OutParallelForMinBatchSize)
{
	...
	bool bIsAllowed = bAllowParallelForDuringIncrementalUpdate && FApp::ShouldUseThreadingForPerformance() && (NumElementsToProcess >= MinNumElementsToProcess);
	OutParallelForMinBatchSize = (bIsAllowed && (GWorkerCountForParallelForDuringIncrementalUpdate > 0) && (NumElementsToProcess > GWorkerCountForParallelForDuringIncrementalUpdate)) ? FMath::DivideAndRoundUp(NumElementsToProcess, GWorkerCountForParallelForDuringIncrementalUpdate) : 1;
	return bIsAllowed;
#else
