				...
				if (AActor* OwningActor = PrimComp->GetOwner())
				{
					...
					if (bIsSimulated || bIsReplicatedAutonomous)
					{
						...
						if (UpdatedState.Flags & ERigidBodyFlags::NeedsUpdate)
						{
							...
							if (PhysicsReplicationCVars::SkipSkeletalRepOptimization == 0)
							{
							...
							else
							{
								...
								// However, we must skip the optimisation if the end physics tick which does that polling is not running (e.g, budgeted meshes)
								const USkeletalMeshComponent* SkeletalPrimComp = Cast<USkeletalMeshComponent>(PrimComp);
								if (!SkeletalPrimComp || !SkeletalPrimComp->IsEndPhysicsTickFunctionEnabled())
								{
									PrimComp->SyncComponentToRBPhysics();
								}
							}
