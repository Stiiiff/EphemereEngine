...
private:
	...
	EScaledTextMode AutoScaleMode;
	float AutoScaleToWidth;
	float AutoScaleToHeight;
};
...
FTextRenderSceneProxy::FTextRenderSceneProxy( UTextRenderComponent* Component) :
	...
	AutoScaleMode(Component->AutoScaleMode),
	AutoScaleToWidth(Component->AutoScaleToWidth),
	AutoScaleToHeight(Component->AutoScaleToHeight)
{
	...
	if(VertexFactory.IsInitialized())
	{
		for (int32 ViewIndex = 0; ViewIndex < Views.Num(); ViewIndex++)
		{
			if (VisibilityMap & (1 << ViewIndex))
			{
				...
				//Draw the auto scale bounds
				if (AutoScaleMode != EScaledTextMode::None 
					&& (AutoScaleToWidth > 0.0f || AutoScaleToHeight > 0.0f) 
					&& IsSelected())
				{
					const FMatrix& WorldMatrix = GetLocalToWorld();
					const FVector Origin = WorldMatrix.GetOrigin();
					FVector Forward, Right, Up;
					WorldMatrix.GetUnitAxes(Forward, Right, Up);

					const FVector2D Bounds2D(GetLocalBounds().BoxExtent.Y * 2.0f, GetLocalBounds().BoxExtent.Z * 2.0f);

					FVector2D Size2D = Bounds2D;
					FVector2D Offset2D = -Size2D * 0.5f;

					if (AutoScaleToWidth > 0)
					{
						Size2D.X = FMath::Max(AutoScaleToWidth, Bounds2D.X);
						Offset2D.X = -Size2D.X * 0.5f;
						Offset2D.X += HorizontalAlignment == EHTA_Right ? (Size2D.X) : HorizontalAlignment == EHTA_Center ? (Size2D.X) * 0.5f : 0;
					}

					if (AutoScaleToHeight > 0)
					{
						Size2D.Y = FMath::Max(AutoScaleToHeight, Bounds2D.Y);
						Offset2D.Y = -Size2D.Y * 0.5f;
						Offset2D.Y += VerticalAlignment == EVRTA_TextBottom ? (Size2D.Y - Bounds2D.Y) : VerticalAlignment == EVRTA_TextCenter ? (Size2D.Y - Bounds2D.Y) * 0.5f : 0;
					}

					const FVector2D BL = Offset2D - Size2D * 0.5f;
					const FVector2D BR = BL + FVector2D(Size2D.X,0.0f);
					const FVector2D TL = BL + FVector2D(0.0f, Size2D.Y);
					const FVector2D TR = BR + FVector2D(0.0f, Size2D.Y);

					const FVector wBL = WorldMatrix.TransformPosition({ 0, BL.X, BL.Y });
					const FVector wBR = WorldMatrix.TransformPosition({ 0, BR.X, BR.Y });
					const FVector wTL = WorldMatrix.TransformPosition({ 0, TL.X, TL.Y });
					const FVector wTR = WorldMatrix.TransformPosition({ 0, TR.X, TR.Y });

					const FColor HColor = AutoScaleToWidth > 0 ? FColor::Magenta : FColor::White;
					const FColor VColor = AutoScaleToHeight > 0 ? FColor::Magenta : FColor::White;

					auto PDI = Collector.GetPDI(ViewIndex);
					PDI->DrawLine(wBL, wTL, HColor, SDPG_Foreground);
					PDI->DrawLine(wBR, wTR, HColor, SDPG_Foreground);
					PDI->DrawLine(wBL, wBR, VColor, SDPG_Foreground);
					PDI->DrawLine(wTL, wTR, VColor, SDPG_Foreground);
				}
#endif
			}
		}
...
FMatrix UTextRenderComponent::GetRenderMatrix() const
{
	...
	if(!Text.IsEmpty() && Font)
	{
		...
		FMatrix ComponentMatrix = GetComponentTransform().ToMatrixWithScale();

		if (AutoScaleMode != EScaledTextMode::None && (AutoScaleToWidth > 0.0f || AutoScaleToHeight > 0.0f))
		{
			const FVector LocalSize = GetTextLocalSize();

			float RelativeScale = 0;

			if (AutoScaleToWidth > 0)
			{
				RelativeScale = LocalSize.Y / AutoScaleToWidth;
			}

			if (AutoScaleToHeight > 0)
			{
				RelativeScale = FMath::Max(RelativeScale, LocalSize.Z / AutoScaleToHeight);
			}

			if (RelativeScale > 0)
			{
				//Scale down
				if (RelativeScale > 1.0f && AutoScaleMode != EScaledTextMode::UpOnly)
				{
					ComponentMatrix = ComponentMatrix.ApplyScale(1.0f / RelativeScale);
				}
				//Scale up
				else if (RelativeScale < 1.0f && AutoScaleMode != EScaledTextMode::DownOnly)
				{
					ComponentMatrix = ComponentMatrix.ApplyScale(1.0f / RelativeScale);
				}
			}
		...
		return VerticalTransform * ComponentMatrix;
...
void UTextRenderComponent::SetAutoScale(EScaledTextMode Mode, float ToWidth, float ToHeight)
{
	AutoScaleMode = Mode;
	AutoScaleToWidth = ToWidth;
	AutoScaleToHeight = ToHeight;
	MarkRenderStateDirty();
}
