...
bool UStaticMeshComponent::DoesCollisionNameExists(FString CollisionName)
{
	return GetCollisionNames().Contains(CollisionName);
}

TArray<FString> UStaticMeshComponent::GetCollisionNames()
{
	...
	FKShapeElem* ShapeElement;
	int32 ElementCount = GetBodySetup()->AggGeom.GetElementCount();
	...
	{
		ShapeElement = GetBodySetup()->AggGeom.GetElement(0);

		if (ShapeElement)
			FoundNames.Add(ShapeElement->GetName().ToString());
	}
	else
	{
		for (int i = 0; i < GetBodySetup()->AggGeom.GetElementCount(); i++)
    	{
			ShapeElement = GetBodySetup()->AggGeom.GetElement(i);
...
FString UStaticMeshComponent::GetCollisionName(int32 ElementIndex)
{
	if (ElementIndex == -1)
		ElementIndex = 0;
	...
	// Check if there are any collision elements and if the index is valid
	if (GetBodySetup()->AggGeom.GetElementCount() > ElementIndex) // Use > OtherBodyIndex for 0-based index check
	{
		// Directly get the shape element
		if (FKShapeElem* ShapeElement = GetBodySetup()->AggGeom.GetElement(ElementIndex))
		{
			return ShapeElement->GetName().ToString(); // Return name directly if found
		}
	}

	// If no name was found after all checks, return empty FString
	return FString();
}
