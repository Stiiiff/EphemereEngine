...
void USceneCaptureComponent::OnRegister()
{
	...
	if (AActor* MyOwner = GetOwner())
	{
		if (ProxyMeshComponent == nullptr)
		{
			...
			ProxyMeshComponent->SetStaticMesh(Cast<UStaticMesh>(CaptureMeshPath.TryLoad()));
...
#if WITH_EDITORONLY_DATA
		//Set the Soft Path that will get resolved on instantiation.
		CaptureMeshPath.SetPath(TEXT("/Engine/EditorMeshes/MatineeCam_SM.MatineeCam_SM"));
#endif
	}
}
...
#if WITH_EDITORONLY_DATA
	//Set the Soft Path that will get resolved on instantiation.
	CaptureMeshPath.SetPath(TEXT("/Engine/EditorMeshes/PlanarReflectionPlane.PlanarReflectionPlane"));
	CaptureMaterialPath.SetPath(TEXT("/Engine/EditorMeshes/ColorCalibrator/M_ChromeBall.M_ChromeBall"));
#endif
}
...
void UPlanarReflectionComponent::OnRegister()
{
	...
	if (ProxyMeshComponent)
	{
		ProxyMeshComponent->SetMaterial(0, Cast<UMaterial>(CaptureMaterialPath.TryLoad()));
