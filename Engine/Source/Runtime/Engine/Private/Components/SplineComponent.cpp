...
void FSplineCurves::UpdateSpline(bool bClosedLoop, bool bStationaryEndpoints, int32 ReparamStepsPerSegment, bool bLoopPositionOverride, float LoopPosition, const FVector& Scale3D)
{
	...
	// Fix twisted splines due to wrong rotation quaternions: After calculating the position autotangents, we need to enforce the rotation to be aligned to those tangents
	// as they are not automatically updated when points are moved. This leads to always wrong interpolated quaternions, causing rotation flips in USplineComponent::GetQuaternionAtSplineInputKey().
	for (int32 PointIndex = 0; PointIndex < NumPoints; PointIndex++)
	{		
		// Averaging the tangents in case we have a CIM_CurveBreak tangents as we can't have 2 rotations per point so we align it to the average of both.
		const FVector AvgTangent = Position.Points[PointIndex].ArriveTangent.GetSafeNormal() + Position.Points[PointIndex].LeaveTangent.GetSafeNormal();
		Rotation.Points[PointIndex].OutVal = FRotationMatrix::MakeFromXZ(AvgTangent, Rotation.Points[PointIndex].OutVal.GetUpVector()).ToQuat();

		// Make sure the rotation takes the shortest path to the previous point. This is important for calculating the rotation tangents for the cubic blending.
		// Otherwise, the rotation gets still interpolated along the longer path in certain situations.
		// This could also be done in TQuat<T>::CalcTangents() instead
		const FQuat PrevRotation = PointIndex > 0 ? Rotation.Points[PointIndex - 1].OutVal : FQuat::Identity;
		Rotation.Points[PointIndex].OutVal.EnforceShortestArcWith(PrevRotation);
	}
