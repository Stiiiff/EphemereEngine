...
public:
	...
	/** Try to find if the collision with a given name exists */
	UFUNCTION(BlueprintPure, Category="Static Mesh")
	bool DoesCollisionNameExists(FString CollisionName);

	/** Try to return the name list of the collisions */
	UFUNCTION(BlueprintPure, Category="Static Mesh")
	TArray<FString> GetCollisionNames();

	/** Try to return the name of the collision */
	UFUNCTION(BlueprintPure, Category="Static Mesh")
	FString GetCollisionName(int32 ElementIndex);
