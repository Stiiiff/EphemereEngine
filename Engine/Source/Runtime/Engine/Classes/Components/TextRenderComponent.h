...
UENUM(BlueprintType)
enum class EScaledTextMode : uint8
{
	/** Disable Scaling */
	None,
	/** Will only make the content smaller to fit */
	DownOnly,
	/** Will only make the content larger to fit */
	UpOnly,
	/** Will scale the content up or down to match the target size. */
	Both
};
...
class ENGINE_API UTextRenderComponent : public UPrimitiveComponent
{
	...
	//Scale mode to apply in component space
	UPROPERTY(EditAnywhere, BlueprintReadOnly, Category="Text|AutoScale")
	EScaledTextMode AutoScaleMode = EScaledTextMode::None;

	//Width in component space to scale
	UPROPERTY(EditAnywhere, BlueprintReadOnly, Category = "Text|AutoScale")
	float AutoScaleToWidth = 0;

	//Height in component space to scale
	UPROPERTY(EditAnywhere, BlueprintReadOnly, Category = "Text|AutoScale")
	float AutoScaleToHeight = 0;
	...
	/** Set the auto scale mode for this text */
	UFUNCTION(BlueprintCallable, Category = "Rendering|Components|TextRender")
	void SetAutoScale(EScaledTextMode Mode, float ToWidth, float ToHeight);
