...
UObject* StaticDuplicateObjectEx( FObjectDuplicationParameters& Parameters )
{
	...
	const bool bShouldForceTaggedPropertySerialization = Parameters.SourceObject->GetClass() != Parameters.DestClass;
	...
	if (bShouldForceTaggedPropertySerialization)
	{
		Writer.SetUseUnversionedPropertySerialization(false);
	}
	...
	if (bShouldForceTaggedPropertySerialization)
	{
		Reader.SetUseUnversionedPropertySerialization(false);
	}
