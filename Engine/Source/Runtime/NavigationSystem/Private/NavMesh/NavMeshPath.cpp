...
namespace
{
	...
	FORCEINLINE bool CheckVisibility(const FPathPointInfo* StartPoint, const FPathPointInfo* EndPoint,  TArray<FNavigationPortalEdge>& PathCorridorEdges, float OffsetDistance, FPathPointInfo* LastVisiblePoint)
	{
		...
		const FVector RayNormal = (StartTrace-EndTrace) .GetSafeNormal() * OffsetDistance;
		...
		while (CurrentEdge <= EndPointEdge)
		{
			...
			if ( !bIntersected)
			{
				...
				const float Distance = FMath::Min(OffsetDistance, EdgeHalfLength) *  0.1;
