...
struct GAMEPLAYTAGS_API FGameplayTagContainer
{
	...
	/** Returns abbreviated human readable Tag list without parents or property names. If bQuoted is true it will quote each tag */
	FString ToStringSimple(bool bQuoted = false) const;

	/** Returns abbreviated human readable Tag list without parents or property names, but will limit each string to specified len.  This is to get around output restrictions*/
