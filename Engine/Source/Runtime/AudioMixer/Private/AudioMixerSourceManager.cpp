	...
	**************************************************************************/
	...
	void FMixerSourceManager::ComputeOutputBuffersForIdRange(const bool bGenerateBuses, const int32 SourceIdStart, const int32 SourceIdEnd)
	{
		...
		for (int32 SourceId = SourceIdStart; SourceId < SourceIdEnd; ++SourceId)
		{
			...
			// if we're writing to another bus we have to write back to our bus's buffer here
			// so that the bus pulling from it gets post FX in the next pass
			// 
			// Withouth this, FX on a clip inside a scene don't get applied
			if (bIsSourceBus && SourceInfo.IsRenderingToSubmixes() && bGenerateBuses)
			{
				const TSharedPtr<FMixerAudioBus> Bus = AudioBuses.FindRef(SourceInfo.AudioBusId);
				const float* RESTRICT BusBufferPtr = Bus->GetCurrentBusBuffer();

				FMemory::Memcpy((float*)BusBufferPtr, SourceInfo.PreDistanceAttenuationBuffer.GetData(), sizeof(float) * SourceInfo.PreDistanceAttenuationBuffer.Num());
