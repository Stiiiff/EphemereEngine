...
namespace UnrealBuildTool
{
	...
	class UEBuildTarget
	{
		...
		private void SetupGlobalEnvironment(UEToolChain ToolChain, CppCompileEnvironment GlobalCompileEnvironment, LinkEnvironment GlobalLinkEnvironment)
		{
			...
			if (Rules.bCompileCEF3 && (Platform == UnrealTargetPlatform.Win64 || Platform == UnrealTargetPlatform.Linux))
			{
			...
			if (!bUseSharedBuildEnvironment || bCompileMonolithic)
			{
				...
				{
					IsCurrentPlatform = (UEBuildPlatform.IsPlatformInGroup(Platform, UnrealPlatformGroup.Unix) && Platform == BuildHostPlatform.Current.Platform);
				}
				else
				{
