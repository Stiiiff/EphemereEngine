...
namespace UnrealBuildTool
{
	...
	public class WindowsTargetRules
	{
		...
		{
			get { return Compiler == WindowsCompiler.VisualStudio2022 || Compiler == WindowsCompiler.Clang; }
		}
...
//			if(!Target.bFormalBuild && !Target.bUseFastPDBLinking.HasValue && Target.WindowsPlatform.Compiler >= WindowsCompiler.VisualStudio2022)
//			{
...
//			}
		...
		internal static WindowsCompiler GetDefaultCompiler(FileReference ProjectFile)
		{
			...
			if (TryGetVSInstallDir(WindowsCompiler.VisualStudio2022, out VSInstallDir))
			{
		...
		static Dictionary<VersionNumber, ToolChainInstallation> FindToolChainInstallations(WindowsCompiler Compiler)
		{
			...
			if(!CachedToolChainInstallations.TryGetValue(Compiler, out ToolChainInstallations))
			{
			    ...
			    if (BuildHostPlatform.Current.Platform == UnrealTargetPlatform.Win64)
			    {
				    ...
				    else if(Compiler == WindowsCompiler.VisualStudio2022)
				    {
		...
		public static bool TryGetNetFxSdkInstallDir(out DirectoryReference OutInstallDir)
		{
			...
			string[] PreferredVersions = new string[] { "4.6.2", "4.6.1", "4.6.0" };
