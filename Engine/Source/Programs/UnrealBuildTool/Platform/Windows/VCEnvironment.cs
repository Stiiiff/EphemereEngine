...
namespace UnrealBuildTool
{
	...
	class VCEnvironment
	{
		...
		private void SetupEnvironment(UnrealTargetPlatform Platform)
		{
			...
			if (ToolChain >= WindowsCompiler.VisualStudio2022)
			{
			...
			if(ToolChain >= WindowsCompiler.VisualStudio2022 && WindowsSdkVersion < new VersionNumber(10))
			{
		...
		public static VCEnvironment Create(WindowsCompiler Compiler, UnrealTargetPlatform Platform, WindowsArchitecture Architecture, string CompilerVersion, string WindowsSdkVersion, string SuppliedSdkDirectoryForVersion)
		{
			...
			{
				if (WindowsPlatform.TryGetToolChainDir(WindowsCompiler.VisualStudio2022, null, out SelectedToolChainVersion, out SelectedToolChainDir))
				{
