...
namespace UnrealBuildTool
{
	class VCToolChain : ISPCToolChain
	{
		...
		protected virtual void AppendCLArguments_Global(CppCompileEnvironment CompileEnvironment, List<string> Arguments)
		{
			...
			if (Target.WindowsPlatform.Compiler == WindowsCompiler.Clang)
			{
				// Tell the Clang compiler whether we want to generate 64-bit code
			...
			if (Target.WindowsPlatform.Compiler >= WindowsCompiler.VisualStudio2022 && Target.WindowsPlatform.bStrictConformanceMode)
			{
			...
			if (Target.WindowsPlatform.Compiler >= WindowsCompiler.VisualStudio2022 || Target.WindowsPlatform.Compiler == WindowsCompiler.Clang)
			{
