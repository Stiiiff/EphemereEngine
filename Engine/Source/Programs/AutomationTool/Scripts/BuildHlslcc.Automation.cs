...
class BuildHlslcc : BuildCommand
{
	...
	private List<WindowsCompiler> GetTargetWindowsCompilers()
	{
		...
		string TargetWindowsCompilersFilter = ParseParamValue("TargetWindowsCompilers", "VisualStudio2022");
	...
	private static string GetMsBuildExe(WindowsCompiler Version)
	{
		...
		{
			case WindowsCompiler.VisualStudio2022:
				VisualStudioToolchainVersion = "17.0";
				break;
		}
	...
	private static void SetupStaticBuildEnvironment()
	{
		if (!Utils.IsRunningOnMono)
		{
			...
			if (VS2022Path != null)
			{
				MsDev14Exe = new FileReference(GetMsDevExe(WindowsCompiler.VisualStudio2022));
