	...
	{
		{
			...
			{
				EXPECT_NE(Particles.X(8), (Chaos::TVector<FReal, 3>)CulledParticles[Idx]);	//interior particle gone
				bool bFound = false;
				for (uint32 InnerIdx = 0; InnerIdx < Particles.Size(); ++InnerIdx)	//remaining particles are from the original set
				{
					if (Particles.X(InnerIdx) == (Chaos::TVector<FReal,3>)CulledParticles[Idx])
					{
						bFound = true;
						break;
					}
