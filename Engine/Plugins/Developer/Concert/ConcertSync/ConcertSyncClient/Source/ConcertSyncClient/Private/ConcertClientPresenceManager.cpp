...
void FConcertClientPresenceManager::SynchronizePresenceState()
{
	...
	for (auto It = PresenceStateMap.CreateIterator(); It; ++It)
	{
		...
		if (bShowPresence)
		{
			...
			{
				PresenceState.PresenceActor = CreatePresenceActor(ClientSessionInfo.ClientInfo);
			}
			else if (PresenceState.PresenceActor->GetClass()->GetPathName() != ClientSessionInfo.ClientInfo.DesktopAvatarActorClass)		
			{
...
AConcertClientPresenceActor* FConcertClientPresenceManager::CreatePresenceActor(const FConcertClientInfo& InClientInfo )
{
	AConcertClientPresenceActor* PresenceActor = SpawnPresenceActor(InClientInfo);
...
AConcertClientPresenceActor* FConcertClientPresenceManager::SpawnPresenceActor(const FConcertClientInfo& InClientInfo)
{
	...
	{
		UE_LOG(LogConcert, Warning, TEXT("Failed to load presence actor class '%s'. Presence will not be displayed"), *InClientInfo.DesktopAvatarActorClass);
		return nullptr;
	}
	...
	if (!PresenceActor)
	{
		UE_LOG(LogConcert, Warning, TEXT("Failed to spawn presence actor of class '%s'. Presence will not be displayed"), *InClientInfo.DesktopAvatarActorClass);
		return nullptr;
	}

	// Setup the asset container.
	PresenceActor->InitPresence(*AssetContainer, FName());
	{
...
void FConcertClientPresenceManager::UpdatePresenceAvatar(const FGuid& InEndpointId)
{
