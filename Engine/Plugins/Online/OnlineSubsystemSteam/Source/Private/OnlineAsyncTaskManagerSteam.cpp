...
public:
	...
	virtual void Finalize() override
	{
		...
		if (SessionInt.IsValid())
		{
			...
			if (Session)
			{
				int32 OldOpenPubConn = Session->NumOpenPublicConnections;
				...
				//Now we update the open connections flag on steam, to properly represent the actual value.
				//Check if we are the owner of the lobby.
				if (SteamUser()->GetSteamID() == SteamMatchmaking()->GetLobbyOwner(CallbackResults.m_ulSteamIDLobby))
				{
					//this value should represent the correct open connection count, because FillMembersFromLobbyData parsed the value.
					int32 NewOpenPubConn = Session->NumOpenPublicConnections;
					if (NewOpenPubConn != OldOpenPubConn)
					{
						SteamMatchmaking()->SetLobbyData(*LobbyId, STEAMKEY_NUMOPENPUBLICCONNECTIONS, TCHAR_TO_UTF8(*FString::FromInt(NewOpenPubConn)));
						UE_LOG_ONLINE(Verbose, TEXT("Updating open connections from %d to %d"), OldOpenPubConn, NewOpenPubConn);
					}
				}
			}
			else
