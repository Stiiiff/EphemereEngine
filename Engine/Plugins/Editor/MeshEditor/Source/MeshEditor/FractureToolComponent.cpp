...
void UFractureToolComponent::OnFractureLevelChanged(uint8 ViewLevelIn)
{
	...
	if (GeometryCollectionComponent)
	{
		...
		if (RestCollection)
		{
			// comment out this line as EditableMesh is no longer part of the rest collection 
			// ResetBoneSelection call does not rely on it so this should have no side effect 
