...
bool PointLightUpdateCommand::Update()
{
	...
	PointLightUsing.PointLight->GetLightComponent()->SetLightBrightness(PointLightUsing.LightBrightness);
	PointLightUsing.PointLight->GetLightComponent()->SetLightColor(PointLightUsing.LightColor);
	PointLightUsing.PointLight->TeleportTo(PointLightUsing.LightLocation, FRotator(0, 0, 0));
	ULocalLightComponent* dd = Cast<ULocalLightComponent>(PointLightUsing.PointLight->GetLightComponent());
	dd->SetAttenuationRadius(PointLightUsing.LightRadius);
	return true;
}
...
bool FStaticMeshValidation::RunTest(const FString& Parameters)
{
	...
	DirectionalLight->GetLightComponent()->SetLightBrightness(3.142f);
	DirectionalLight->GetLightComponent()->SetLightColor(FColor::White);
