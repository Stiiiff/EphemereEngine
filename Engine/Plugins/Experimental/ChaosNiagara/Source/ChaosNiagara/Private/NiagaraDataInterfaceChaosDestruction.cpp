...
void UNiagaraDataInterfaceChaosDestruction::HandleTrailingEvents(const Chaos::FTrailingEventData& Event)
{
	...
	if (GeometryCollectionComponentsFromBreaking.Num() > 0)
	{
		...
		{
			//const bool bIsValidComponent = GeometryCollectionComponent && GeometryCollectionComponent->IsBeingDestroyed();
			if (GeometryCollectionComponent)
			{
					...
					if (DynamicCollection)
					{
						if (const UGeometryCollection* RestCollection = GeometryCollectionComponent->GetRestCollection())
						{
							...
							if (GeometryCollection->HasAttribute(TEXT("MassToLocal"), FTransformCollection::TransformGroup))
							{
								...
								const TManagedArray<FVector>* LinearVelocity = DynamicCollection->FindAttributeTyped<FVector>("LinearVelocity", FTransformCollection::TransformGroup);
								const TManagedArray<FVector>* AngularVelocity = DynamicCollection->FindAttributeTyped<FVector>("AngularVelocity", FTransformCollection::TransformGroup);
								...
								{
									if (DynamicCollection->Active[Idx] && (*LinearVelocity)[Idx].SizeSquared() >= TrailMinSpeedToSpawnSquared)
									{
