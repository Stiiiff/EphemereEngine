...
int32 UFractureToolConvert::ExecuteFracture(const FFractureToolContext& FractureContext)
{
	if (FractureContext.IsValid())
	{
		...
		TArray<UMaterialInterface*>& OverrideMaterials = FractureContext.GetGeometryCollectionComponent()->OverrideMaterials;
		TArray<UMaterialInterface*>& MainMaterials = FractureContext.GetFracturedGeometryCollection()->Materials;
		TArray<UMaterialInterface*> Materials;
		Materials.Reserve(MainMaterials.Num());
		for (int MatIdx = 0; MatIdx < MainMaterials.Num(); MatIdx++)
		{
			if (MatIdx < OverrideMaterials.Num() && OverrideMaterials[MatIdx] != nullptr)
			{
				Materials.Add(OverrideMaterials[MatIdx]);
			}
			else
			{
				Materials.Add(MainMaterials[MatIdx]);
			}
		}
