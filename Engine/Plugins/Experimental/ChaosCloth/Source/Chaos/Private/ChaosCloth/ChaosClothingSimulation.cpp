...
void FClothingSimulation::Simulate(IClothingSimulationContext* InContext)
{
...
#endif
	{
		...
		Solver->SetVelocityScale(Context->VelocityScale);
	...
	if (ChaosClothingSimulationCVar::DebugDrawPointVelocities     .GetValueOnAnyThread()) { DebugDrawPointVelocities     (); }
	...
	for (const TUniquePtr<FClothingSimulationCloth>& Cloth : Cloths)
	{
		...
		if (bRealTypeCompatibleWithISPC && bChaos_GetSimData_ISPC_Enabled)
		{
			...
			ispc::GetClothingSimulationData(
				...
				(ispc::FVector&)FVector::ZeroVector,
				Data.Positions.Num());
#endif
		}
		else
		{
			...
			{
				Data.Positions[Index] = ReferenceSpaceTransform.InverseTransformPosition(Data.Positions[Index]);  // Move into world space first
...
void FClothingSimulation::DebugDrawPointVelocities(FPrimitiveDrawInterface* PDI) const
{
	...
	for (const FClothingSimulationCloth* const Cloth : Solver->GetCloths())
	{
		...
		const TConstArrayView<FVec3> Positions = Cloth->GetParticlePositions(Solver.Get());
		const TConstArrayView<FVec3> Velocities = Cloth->GetParticleVelocities(Solver.Get());
		...
		for (int32 Index = 0; Index < Positions.Num(); ++Index)
		{
			...
			const FVector Pos0 = LocalSpaceLocation + Positions[Index];
			const FVector Pos1 = Pos0 + Velocities[Index] / DefaultFPS;  // Velocity per frame if running at DefaultFPS
