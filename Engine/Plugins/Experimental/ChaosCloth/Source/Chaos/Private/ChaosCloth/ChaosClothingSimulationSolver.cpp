...
static bool bChaosClothSolverUseVelocityScale = true;
...
FAutoConsoleVariableRef CVarChaosClothSolverUseVelocityScale(TEXT("p.ChaosCloth.Solver.UseVelocityScale"), bChaosClothSolverUseVelocityScale, TEXT("Use the velocity scale to compensate for clamping to MaxPhysicsDelta, in order to avoid miscalculating velocities during hitches."));
	...
	FReal FictitiousAngularScale)
{
	...
	const FReal SolverVelocityScale = bChaosClothSolverUseVelocityScale ? VelocityScale : (FReal)1.;
	...
	const FVec3 LinearRatio = FVec3(1.f) - LinearVelocityScale.BoundToBox(FVec3(0.f), FVec3(1.f)) * SolverVelocityScale;
	...
	const FReal PartialDeltaAngle = DeltaAngle * FMath::Clamp(1.f - AngularVelocityScale * SolverVelocityScale, 0.f, 1.f);
