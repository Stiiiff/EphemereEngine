...
void FClothingSimulationCloth::FLODData::Add(FClothingSimulationSolver* Solver, FClothingSimulationCloth* Cloth, int32 InLODIndex)
{
	...
	const TConstArrayView<FRealSingle> TetherScaleMultipliers;  // Not using scale multipliers on 4.27-Chaos
...
void FClothingSimulationCloth::FLODData::Update(FClothingSimulationSolver* Solver, FClothingSimulationCloth* Cloth)
{
	...
	ClothConstraints.SetLongRangeAttachmentProperties(Cloth->TetherStiffness, FVec2((FReal)Cloth->LimitScale * MeshScale));
...
TConstArrayView<FVec3> FClothingSimulationCloth::GetParticleVelocities(const FClothingSimulationSolver* Solver) const
{
	check(Solver);
	const int32 LODIndex = LODIndices.FindChecked(Solver);
	check(GetOffset(Solver, LODIndex) != INDEX_NONE);
	return TConstArrayView<FVec3>(Solver->GetParticleVs(GetOffset(Solver, LODIndex)), GetNumParticles(LODIndex));
}
